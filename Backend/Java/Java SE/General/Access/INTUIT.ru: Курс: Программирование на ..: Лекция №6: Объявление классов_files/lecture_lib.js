...
function soundPlay(which)
    {
//	var dir = '/department/humanities/enforit/1/';
	var dir = '';
    if (!soundEmbed)
        {
        soundEmbed = document.createElement("embed");
        soundEmbed.setAttribute("src", dir+which);
...
function play_sound_loc(id,word,sss) {
...
function ____play_sound_loc(id,word,sss) {
//	alert(id+'] ['+word);
	var mmm = "lsound_"+id;
	var o = document.getElementById(mmm);
	if(o) {
//		alert("begin: "+word);
//		o.play();
		soundPlay(sss);
...
function play_g_sound(id,word) {
//	alert(id+'] ['+word);
	var o = document.getElementById("sound_"+id);
	if(!o) return;
	var sound = "http://translate.google.ru/translate_tts?tl=en&q="+encodeURI(word);
	var eid = "esound_"+id;
	o.innerHTML=
		"<embed id=\""+eid+"\" src=\""+sound+"\" autostart=\"false\" loop=\"false\" hidden=\"true\" />";
...
function play_g_sound2(id,word) {
//	alert(id+'] ['+word);
	var o = document.getElementById("zsound_"+id);
	if(!o) {
		alert("not found: "+"zsound_"+id);
		return;
	}
	if(o) {
//		alert('play '+eid);
...
function set_exercise_forms () {
//	alert('1');
	var empty = true;
	var q = [];
	var lzzz = 'is_correct_'.length;
	var lexercise = 'exercise_'.length;
	$('div.exercise').each(function(x) {
		var iid = $(this).attr('id');
		var id = iid.substring(lexercise);
//alert ('id='+id+' iid='+iid);
		q.push(id);
		empty = false;
//		alert(id);
...
//		e['object'] = x;
		...
		e['input'] = input;
//		alert(0);
		$(this).find('input[type=checkbox],input[type=radio]').each(function(y) {
//			alert('--1');
...
//			alert('--2');
			...
			input[n] = z; 
z['qqq'] = 'aaa';
			z['checked'] = $(this).attr('checked');
//			alert('--3');
//			z['object'] = y;
			z['object'] = $(this);
//			alert('--4');
		});
		$(this).find('input[type=hidden]').each(function(y) {
			var n = $(this).attr('name');
			var v = $(this).attr('value');
			var id = n.substring(lzzz);
//alert('hidden['+n+']['+id+']('+lzzz+')');
//['+v+']
			if(input[id]) {
//				alert('correct='+v);
				input[id]['correct'] = v == 'yes'? true:false;
			}
		});
	});
...
function handler_button_exercise(code) {
	aa('code='+code);
	var n = exercise_forms[code];
	if(!n) {
		alert("not find code ["+code+"]");
		return;
	}
	var oobject = n['object'];
	var input = n['input'];
	var ok = true;
	for(var k in input) {
		if(input.hasOwnProperty(k) && !(typeof k === "undefined")) {
			var v = input[k];
aa('['+k+'| |'+v+']');
			var o = v['object'];
			var correct = v['correct'];
			var a = false;
			if(o.attr('checked')) a = true;
			if(a != correct) {
				ok = false;
			}
		}
	...
	var n_ok;
	var n_fail;
	oobject.find('div.ok').each(function(z,y) {
		n_ok =y;
	});
	oobject.find('div.fail').each(function(z,y) {
		n_fail=y;
	});
	if(n_ok) n_ok.style.display =d_ok;
	if(n_fail) n_fail.style.display =d_fail;
}
...
function _______on_load_lecture () {
	move_content_lection();
    if(!is_auth) {
	return;
    }
